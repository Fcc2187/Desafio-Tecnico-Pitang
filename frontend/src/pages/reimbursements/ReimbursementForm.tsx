...
import { showSuccessAlert, showErrorAlert, showWarningAlert } from '../../components/ui/alerts';
...
export const ReimbursementForm = ({ onClose, initialData }: ReimbursementFormProps) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showSuccessAlert(
        'Sucesso!',
        isEditing ? 'Solicitação atualizada com sucesso.' : 'Rascunho salvo com sucesso.'
      );
    ...
    onError: (error: any) => {
      showErrorAlert(
        'Erro!',
        error.response?.data?.message || error.message || 'Erro ao salvar a solicitação'
      );
    }
  });
